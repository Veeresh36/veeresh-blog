...
const ArticleTags = ({ tags, dark }) => {
  ...
  return (
    <div className="mt-12 pt-8 flex items-center gap-2.5 flex-wrap" style={{ borderTop: `1px solid ${dark ? "rgba(255,255,255,0.07)" : "#EAE4DC"}` }}>
      ...
      {normalized.map(tag => (
        <span key={tag}
          className="inline-block text-[0.73rem] font-semibold px-3.5 py-1.5 rounded-full border"
          style={{ background: dark ? "rgba(255,255,255,0.05)" : "#F5F1EB", color: dark ? "rgba(250,248,244,0.7)" : "#3D3530", borderColor: dark ? "rgba(255,255,255,0.09)" : "#DDD7CE", cursor: "default" }}>
          {tag}
        </span>
      ))}
    </div>
